...
class HospitalMegaMenu {
  ...
  setupAccessibility() {
    ...
    megaMenus.forEach((menu, index) => {
      ...
      // Make section headers focusable for keyboard navigation
      const headers = menu.querySelectorAll('.mega-menu-header');
      headers.forEach(header => {
        header.setAttribute('tabindex', '0');
        header.setAttribute('role', 'heading');
        header.setAttribute('aria-level', header.tagName.replace('H', ''));
      });
    });
  }
  ...
  hideMegaMenu() {
    ...
    if (megaMenu) {
      ...
      // Restore focusability state when closed
      megaMenu.querySelectorAll('.dropdown-item').forEach(item => {
        item.setAttribute('tabindex', '-1');
      });
    }
